...
class Transport {
  ...
  /**
   * Load a faceplate's full metadata + SVG for rendering. Uses the domain
   * API (`getFaceplateByName` / `getFaceplateSvg` / `getFaceplateElements` /
   * `getFaceplatePlaceholders`) — 4 parallel round-trips in iframe mode,
   * same in standalone.
   *
   *   iframe mode → InscadaApi proxy over postMessage → parent SPA →
   *   /api/scripts/call-api → server-side FaceplateApi. Backend ≥
   *   2026-07-06 build 20260706-8 (commit 617709390) + UI mirror
   *   faceplateApi.js required.
   *
   *   standalone mode → falls through to loadFaceplate() which fetches the
   *   REST endpoints directly with same-origin credentials.
   *
   * Returns:
   *   { def, projectId, svg, elements, placeholders }
   *
   * Throws:
   *   Error if the faceplate is not found (`getFaceplateByName` returns null
   *   → we surface a specific 'Faceplate "X" not found' message).
   *   Any transport error (network / auth / bridge dispatch failure).
   */
  async loadFaceplateFull(projectName, faceplateName) {
    if (!faceplateName) throw new Error('faceplateName is required');
    if (this._isIframeWithProxy) {
      if (!projectName) throw new Error('projectName is required in iframe mode');
      const api = this._getApi(projectName);
      const [def, svg, elements, placeholders] = await Promise.all([
        api.getFaceplateByName(faceplateName),
        api.getFaceplateSvg(faceplateName),
        api.getFaceplateElements(faceplateName),
        api.getFaceplatePlaceholders(faceplateName),
      ]);
      if (!def) throw new Error('Faceplate "' + faceplateName + '" not found in project ' + projectName);
      return {
        def,
    ...
    return this.loadFaceplate(projectName, faceplateName);
  }
